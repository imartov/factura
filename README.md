...
5.2. [Перевод](#перевод)  
5.3. [Фундаментальные правила работы с <b>FacturaPy</b>](#правила_запуск)   
5.4. [Схема](#схема)  
5.5. [Исключения](#исключения)  
5.6. [Система логирования](#логирование) 
...
### <a name='перевод'>5.2. Перевод</a>

В <b>FacturaPy</b> встроен модуль, позволяющий осуществлять языковые переводы наименования товаров с любого языка также на любой язык. По техническому заданию языком перевода является польский. Язык же, с которого нужно перевести, определяется автоматически. Если входным языком является польский, это не будет влечь какие-либо ошибки, а <b>FacturaPy</b> просто обработает исходные текст.


### <a name='правила_запуск'>5.3. Фундаментальные правила работы с <b>FacturaPy</b></a>
...
### <a name='логирование'>5.6. Система логирования</a>

В <b>FacturaPy</b> встроен модуль системы логирования, которая позволяет отслеживать ошибки и сохраняет подробный отчет. При сбое в работе <b>FacturaPy</b> в корневой папке программы будет создан / перезаписан файл <b>debug.log</b>, содержащий подробный отчет о сбое. Если у Вас возникнут сложности с чтением отчета об ошибке и ее устранении, обратитесь к [разработчику](#разработчик) с данным файлом.
